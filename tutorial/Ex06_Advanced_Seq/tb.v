...
module tb_ex06_advanced_seq;
    ...
    Ex05_HelloWorld u_hello(.clk(clk), .rst_n(rst_n), .char(char));
    Ex06_VendingMachine u_vending(.clk(clk), .rst_n(rst_n), .coin_5(coin_5), .coin_10(coin_10), .vend(vend), .change_5(change_5), .amount(amount));
    ...
    initial begin
        ...
        if(errors==0) begin $display("[PASS] Ex06_Advanced_Seq"); $fdisplay(log_fd,"[PASS] Ex06_Advanced_Seq"); $fdisplay(result_fd,"pass"); end
        else begin $display("[FAIL] Ex06_Advanced_Seq, errors = %0d", errors); $fdisplay(log_fd,"[FAIL] Ex06_Advanced_Seq, errors = %0d", errors); $fdisplay(result_fd,"fail"); end
